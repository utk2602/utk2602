...
Env:
  ...
  GH_LOGIN      user to summarise (default: utk2602)
...
def summarise(user):
    ...
    if user["repositories"]["pageInfo"]["hasNextPage"]:
        print("warning: more than 100 public repos — language stats are "
              "truncated to the first page", file=sys.stderr)
...
def draw_stats(s):
    ...
    base, top = H - 10, H - 58
    span = base - top
    # end 6px short of the right edge so the end dot (cx = x - 2, r 4.5,
    # stroke 2) stays inside the viewBox instead of being flattened by it
    step = (WIDTH - 6) / max(len(weekly) - 1, 1)
...
def short_name(name, limit=11):
    """Shorten a language name to the label column, at a word boundary.

    A hard slice reads as a mistake — "Jupyter Notebook"[:11] is "jupyter not".
    Dropping trailing words keeps it sensible; only a single over-long word is
    ever hard-cut.
    """
    name = name.lower()
    while len(name) > limit and " " in name:
    ...
    for gi, (gx, title, data, as_pct) in enumerate(groups):
        ...
        for ri, (name, val) in enumerate(data):
            ...
            p.append(f'<g opacity="0">{fade(0.24 + gi * 0.10 + ri * 0.05)}'
                     + label(gx, y + 8, short_name(name), 11, "e-f")
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "utk2602")
